...
export default function App() {
  ...
  const generarCopia = (startY, esPrimeralCopia) => {
    ...
    doc.addImage(logo, 'PNG', 10, startY + 8, 18, 18);
doc.setFontSize(20);
doc.setTextColor(255, 255, 255);
doc.text("FIX", 32, startY + 15);
doc.setTextColor(255, 122, 26);
doc.text("LAB", 42, startY + 15);
doc.setTextColor(255, 122, 26);
doc.setFontSize(10);
doc.text("Reparación de celulares y PC", 32, startY + 20);
    ...
    doc.setFont(undefined, 'normal');
    doc.text(orden.falla || '', col2X + 12, yPos);
    ...
    doc.setFont(undefined, 'normal');
    doc.text(orden.accesorios || 'no', col2X + 22, yPos);
    ...
    doc.setFont(undefined, 'bold');
    doc.text("IMEI / Serie:", col2X, yPos);
    doc.setFont(undefined, 'normal');
    doc.text(orden.imei || '', col2X + 26, yPos);

    yPos += lineHeight;
    doc.setFont(undefined, 'bold');
    doc.text("Tipo Contraseña:", col1X, yPos);
    doc.setFont(undefined, 'normal');
    doc.text(orden.tipo_contrasena || 'No esp.', col1X + 30, yPos);
    doc.setFont(undefined, 'bold');
    doc.text("Contraseña:", col2X, yPos);
    doc.setFont(undefined, 'normal');
    doc.text(orden.password || 'No registrada', col2X + 24, yPos);

    yPos += lineHeight;
    doc.setFont(undefined, 'bold');
    doc.text("Estado:", col1X, yPos);
    doc.setFont(undefined, 'normal');
    doc.text(orden.estado || '', col1X + 14, yPos);
    doc.setFont(undefined, 'bold');
    doc.text("Importe:", col2X, yPos);
    doc.setFont(undefined, 'normal');
    doc.text(`$${orden.importe || ''}`, col2X + 18, yPos);

    yPos += lineHeight + 2;
    doc.setFont(undefined, 'bold');
    doc.text("Observaciones:", col1X, yPos);
    doc.setFont(undefined, 'normal');
    const obsLines = doc.splitTextToSize(orden.observaciones || 'Sin observaciones', pageWidth - 20);
    doc.text(obsLines, col1X, yPos + lineHeight);
...
function Dashboard({ ordenes, caja, productos, setTab }) {
  ...
  const [expandBalance, setExpandBalance] = useState(true);
